...
func run():
	forces_vector.x = Input.get_action_strength("ui_right_3")*SPEED - Input.get_action_strength("ui_left_3")*SPEED
...
func jump():
	if is_on_floor():
		jumps = 1
		forces_vector.y = 0
		if Input.is_action_just_pressed("ui_up_3"):
			forces_vector.y -= JUMP_FORCE
	else: 
		if Input.is_action_just_pressed("ui_up_3") and jumps > 0:
			jumps -= 1
			forces_vector.y = min(forces_vector.y - JUMP_FORCE, -JUMP_FORCE)
	if forces_vector.y < 0 and Input.is_action_just_released("ui_up_3"):
		forces_vector.y *= 0.5
...
func fire():
	if Input.is_action_pressed("ui_accept_3") and can_fire:
